...
if(!empty($_POST["SendEditname"])&& !empty($_POST["SendEditcomment"]) &&!empty($_POST["SendEditpass"])){
  ...
  foreach ($result as $row){
      if($SendEditnum==intval($row['id'])&&strcmp($SendEditpass,$row['password'])==0){
        ...
        $sql = 'update userinfo set name=:name,comment=:comment where id=:id';
        $stmt = $pdo->prepare($sql);
...
//通常モード処理
  if(!empty($_POST["name"]) && !empty($_POST["comment"]) && !empty($_POST["password"])){
    ...
    $sql = $pdo -> prepare("INSERT INTO userinfo (name, comment, date, password) VALUES (:name, :comment, :date, :password)");
